	...
	public static void main(String[] args) throws IOException {
		...
		for(int h = 0; h<sub.length;h++) { // 반
			for(int k = 0; k<sub[h].length;k++) { // 반 학생
				for(int i = 0; i<sub.length;i++) { // 첫번째
					for(int j = 0;j<sub[i].length;j++) { // 두번째
					    if(sub[h][k][sub[h][k].length-3]<sub[i][j][sub[i][j].length-3]) { // 총점을 기준으로 비교함
